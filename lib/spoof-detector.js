...
function checkHeaders(ethrPacket){
  ...
  if(ethrPacket.shost.addr !==  arpPacket.sender_ha.addr) return false;
  //checking dst mac address
  if(ethrPacket.dhost.addr !== [ 255, 255, 255, 255, 255, 255 ]){//not a gratuitous reply
    if(ethrPacket.dhost.addr !== arpPacket.target_ha.addr) return false;
  }
  return true;
}
...
function handleARPPacket(etherPacket){
  ...
  if(!filterARPPackets(arpPacket)) {logger.log('asd', 'debug');return;}
