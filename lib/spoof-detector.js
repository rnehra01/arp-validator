...
function validateHost(host_ip, host_mac){
  ...
  if(validatedHosts[host_ip] != undefined){ //host is already validated
    if(validatedHosts[host_ip] != host_mac){
      console.log('[-] Validation Failed : %s at %s\nALERT!!! ARP POISONING under Progress', address, pendingSYNs[address]);
    }else{
      console.log('[+] Already Validated: %s is at %s\n', host_ip, validatedHosts[host_ip]);
    }
    return;
  }
...
function handleTCPPacket(ipPacket){
  ...
  if(pendingSYNs[host_ip +':'+ host_port] != undefined){
    ...
    console.log('[+] Validated: %s is at %s\n', host_ip, pendingSYNs[host_ip +':'+ host_port][0]);
